...
def run_data_sanitization(**context):
...
def run_abt_transform(**context):
    ...
    logger = logging.getLogger(__name__)
    logger.info("Executando ABT Transform (Clean -> ABT)...")

    script_path = "/opt/airflow/DataPipeline/abt_transform.py"
    result = subprocess.run(
    ...
    if result.returncode != 0:
        logger.error(f"ABT transform falhou: {result.stderr}")
        raise Exception(f"ABT transform falhou: {result.stderr}")

    logger.info("ABT transform concluída com sucesso")
    logger.info(result.stdout)
    return {"status": "success", "layer": "abt"}
...
sensor_raw_fraud_data_pipeline = ExternalTaskSensor(
    task_id='sensor_raw_fraud_data_pipeline',
    external_dag_id='raw_fraud_data_pipeline', 
